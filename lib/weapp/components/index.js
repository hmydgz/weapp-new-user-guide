  ...
  methods: {
    ...
    setCurrentTargetNodeInfo() {
      ...
      selectNodeRect(item.selector).then((e) => {
        if (e.top > SYSTEM_INFO.windowHeight - 20) { // 需要滚动到可视区域
          ...
          return setTimeout(() => this.setCurrentTargetNodeInfo(), 32)
        }
    ...
    handleStepEnd() {
      ...
      setTimeout(() => {
        this.setData({ show: false, hide: true })
        const obj = wx.getStorageSync(NEW_USER_GUIDE_COMPLETION_STATUS) || {}
    ...
    setTipsStyle() {
      wx.nextTick(async () => {
        ...
        if (SYSTEM_INFO.windowHeight - this.data.targetNodeInfo.bottom < height) {
          this.setData({ tipsStyle: `top: ${this.data.targetNodeInfo.top - height - 20}px;` })
        } else {
